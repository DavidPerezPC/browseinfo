...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self,grouped=False, final=False, date=None):
        ...
        customer_currency_rate_map = {}
        for order in self:
            partner_id = order.partner_id.id
            rate = order.sale_manual_currency_rate
            customer_currency_rate_map.setdefault(partner_id, set()).add(rate)

        for partner_id, rates in customer_currency_rate_map.items():
            if len(rates) > 1:
                raise ValidationError(_("You cannot create invoice when the same customer has multiple different manual currency rates."))
...
class PricelistItem(models.Model):
    ...
    def _compute_price(self, product, quantity, uom, date, currency=None, **kwargs):
        ...
        manual_currency_rate_active = product.env.context.get('manual_currency_rate_active')
        manual_currency_rate = product.env.context.get('manual_currency_rate')
